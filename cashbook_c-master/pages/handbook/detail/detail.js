...
Page({
  ...
  onLoad: function (params) {
    ...
    if (params.act === 'pay') {
      ...
      var t_items = list[params.mainindex].items
      for (var i = t_items.length > 5 ? t_items.length - 5 : 0; i < t_items.length; i++) {
        var t_date = new Date(t_items[i].date.replace(/-/g, '/') + ' ' + t_items[i].time)
        if (distances.has(t_items[i].typeindex)) {
          var tmp = distances.get(t_items[i].typeindex) + 1 / ((Math.abs(d1 - t_date) / 300000) ^ 2)
          distances.delete(t_items[i].typeindex)
          distances.set(t_items[i].typeindex, tmp)
        } else {
          distances.set(t_items[i].typeindex, 1 / ((Math.abs(d1 - t_date) / 300000) ^ 2))
        }
      }
      var max = 0
      var maxindex = 0
      distances.forEach(function (dis, key) {
        if (dis > max) {
          max = dis
          maxindex = key
        }
